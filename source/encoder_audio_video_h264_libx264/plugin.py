...
from encoder_audio_video_h264_libx264.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.encoder_audio_video_h264_libx264")
...
class PluginStreamMapper(StreamMapper):
    def __init__(self):
        super(PluginStreamMapper, self).__init__(logger, ['audio', 'video'])
        self.settings = None
    ...
    def test_stream_needs_processing(self, stream_info: dict):
        ...
        if codec_type == 'audio':
            return stream_info.get('codec_name').lower() not in ['aac']
    ...
    def custom_stream_mapping(self, stream_info: dict, stream_id: int):
        codec_type = stream_info.get('codec_type', '').lower()
        if codec_type == 'audio':
            stream_encoding = ['-c:a:{}'.format(stream_id), 'aac']
